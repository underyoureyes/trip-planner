...
export default function NavigateButton({ destination, label, compact = false }: Props) {
  ...
  if (compact) {
    return (
      <a
        href={urls.google}
        className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-brand-600 text-white text-xs font-semibold rounded-lg active:bg-brand-700 no-underline"
      >
      ...
      {showOptions && (
        <>
          ...
          <div className="absolute bottom-full mb-2 left-0 right-0 bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden z-50">
            ...
            <a
              href={urls.google}
              className="flex items-center gap-3 px-4 py-3.5 active:bg-gray-50 no-underline border-b border-gray-100"
              onClick={() => setShowOptions(false)}
            >
              <span className="text-xl">🗺️</span>
              <div>
                <p className="text-sm font-semibold text-gray-900">Google Maps</p>
                <p className="text-xs text-gray-500">Opens app if installed</p>
              </div>
            </a>
            <a
              href={urls.apple}
            ...
            >
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">Built-in iPhone maps</p>
              </div>
            </a>
            <a
              href={urls.web}
              ...
              className="flex items-center gap-3 px-4 py-3.5 active:bg-gray-50 no-underline"
              onClick={() => setShowOptions(false)}
            >
              <span className="text-xl">🌐</span>
              <div>
                <p className="text-sm font-semibold text-gray-900">Google Maps (web)</p>
