...
export default function NewTripPage() {
  ...
  const [form, setForm] = useState<Partial<IntakeForm>>({
    ...
    budget_per_day_gbp: 150, driving_max_hours: 4, must_include: '', notes: '',
  })
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!res.ok) { const json = await res.json().catch(() => ({})); setError(json.error || 'Failed to create trip'); setSubmitting(false); return }
        ...
        <div className="card space-y-4">
          ...
          <div>
            ...
            <input type="range" min={1} max={8} value={form.num_travellers}
              onChange={e => setForm(f => ({ ...f, num_travellers: Number(e.target.value) }))} className="w-full accent-brand-600" />
            ...
            {interestOptions.map(opt => (
              <button key={opt.id} type="button" onClick={() => toggleInterest(opt.id)}
                className={`px-3 py-2 rounded-xl text-sm font-medium border-2 transition-colors ${
                  (form.interests || []).includes(opt.id) ? 'border-brand-500 bg-brand-50 text-brand-700' : 'border-gray-200 bg-white text-gray-600'
                }`}>{opt.label}</button>
            ))}
          </div>
        </div>
        ...
        <div className="card space-y-4">
          ...
          <div>
            ...
            <input type="range" min={50} max={500} step={25} value={form.budget_per_day_gbp}
              onChange={e => setForm(f => ({ ...f, budget_per_day_gbp: Number(e.target.value) }))} className="w-full accent-brand-600" />
          ...
          <div>
            ...
            <input type="range" min={1} max={10} value={form.driving_max_hours}
              onChange={e => setForm(f => ({ ...f, driving_max_hours: Number(e.target.value) }))} className="w-full accent-brand-600" />
        ...
        <div className="card space-y-4">
          ...
          <div>
            ...
            <textarea value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))}
              className="input-field resize-none" rows={3} placeholder="e.g. vegetarian-friendly restaurants, avoid motorways" />
          </div>
        </div>
