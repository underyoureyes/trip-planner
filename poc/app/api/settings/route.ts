...
export async function GET() {
  ...
  const { data, error } = await supabase.from('user_settings')
    .select('distance_unit, currency, claude_api_key').eq('user_id', user.id).single()
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })

  const maskedKey = data?.claude_api_key ? data.claude_api_key.slice(0, 14) + '…' : null
...
export async function PUT(request: NextRequest) {
  ...
  if (typeof body.distance_unit === 'string') {
    if (!['metric', 'imperial'].includes(body.distance_unit)) return NextResponse.json({ error: 'Invalid distance_unit' }, { status: 400 })
  ...
  if (typeof body.claude_api_key === 'string') update.claude_api_key = body.claude_api_key.trim() || null
