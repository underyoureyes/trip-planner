...
export async function POST(request: NextRequest) {
  ...
  if (!settings?.claude_api_key) {
    return NextResponse.json({ error: 'A Claude API key is required to create trips. Add one in settings.' }, { status: 403 })
  }

  let form: Partial<IntakeForm>
  try { form = await request.json() } catch { return NextResponse.json({ error: 'Invalid JSON' }, { status: 400 }) }
  ...
  const { data: trip, error } = await supabase.from('trips').insert({
    owner_id: user.id, title: form.title.trim(), status: 'draft',
    start_date: form.start_date || null, end_date: form.end_date || null,
    is_shared: false, intake_form: form,
  }).select('id').single()

  if (error || !trip) return NextResponse.json({ error: error?.message || 'Insert failed' }, { status: 500 })
