...
export async function POST(_request: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { data: trip, error: tripError } = await supabase.from('trips')
    .select('id, owner_id, status, intake_form').eq('id', params.id).single()
  ...
  if (!settings?.claude_api_key) return NextResponse.json({ error: 'No Claude API key configured' }, { status: 403 })
    ...
    async start(controller) {
      ...
      try {
        ...
        for await (const chunk of streamTripGeneration(settings.claude_api_key, trip.intake_form as IntakeForm)) {
          rawJson += chunk; chunkCount++
          if (chunkCount % 50 === 0) send({ type: 'progress', message: `Generating… (${Math.round(rawJson.length / 1000)}k chars)` })
        }
        send({ type: 'progress', message: 'Parsing itinerary…' })
        let tripData
        try { tripData = parseTripJson(rawJson) }
        catch { await supabase.from('trips').update({ status: 'error' }).eq('id', params.id); send({ type: 'error', message: 'Failed to parse Claude response. Please retry.' }); controller.enqueue(encoder.encode('data: [DONE]\n\n')); controller.close(); return }

        const { error: saveError } = await supabase.from('trip_data').upsert({ trip_id: params.id, data: tripData, version: 1 })
        if (saveError) { await supabase.from('trips').update({ status: 'error' }).eq('id', params.id); send({ type: 'error', message: 'Failed to save: ' + saveError.message }); controller.enqueue(encoder.encode('data: [DONE]\n\n')); controller.close(); return }

        const firstDay = tripData.days?.[0]; const lastDay = tripData.days?.[tripData.days.length - 1]
