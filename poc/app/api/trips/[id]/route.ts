...
export async function GET(_request: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (trip.owner_id !== user.id && !trip.is_shared) return NextResponse.json({ error: 'Forbidden' }, { status: 403 })

  const { data: tripDataRow } = await supabase.from('trip_data').select('data').eq('trip_id', params.id).single()
  return NextResponse.json({ trip, tripData: tripDataRow?.data || null })
}
...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (!trip || trip.owner_id !== user.id) return NextResponse.json({ error: 'Forbidden' }, { status: 403 })
  ...
  const { error } = await supabase.from('trips').update(allowedFields).eq('id', params.id)
